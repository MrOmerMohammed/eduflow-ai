...
export default async function CommunicationPage({searchParams}:{searchParams:Promise<{schoolId?:string}>}) {
 const supabase=await createSupabaseServerClient(); const {data:claims}=await supabase.auth.getClaims();
 if(!claims?.claims?.sub) redirect("/auth/login"); const userId=String(claims.claims.sub);
 ...
 const requested=(await searchParams).schoolId; const membership=(memberships??[]).find(m=>m.school_id===requested)??memberships?.[0];
 ...
 return <CommunicationBoard schoolId={membership.school_id} school={{name:school.name,code:school.code}} />;
}