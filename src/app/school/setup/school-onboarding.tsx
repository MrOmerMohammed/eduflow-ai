...
export function SchoolOnboarding({schoolId,schoolName,initialAnswers,initialStep=1}:{schoolId:string;schoolName:string;initialAnswers?:Partial<Answers>|null;initialStep?:number}){
  ...
  const [step,setStep]=useState(Math.max(1,Math.min(8,Number(initialStep||1))));
