...
export default async function FinancePage({ searchParams }: { searchParams: Promise<{ schoolId?: string }> }) {
  ...
  const [{ data: years }, { data: grades }] = await Promise.all([
    ...
    supabase.from("grades").select("id,name,code").eq("school_id", membership.school_id).order("sort_order").order("name")
  ]);
  return <FinanceBoard schoolId={membership.school_id} school={{ name: school.name, code: school.code }} years={years ?? []} grades={grades ?? []} />;
}
